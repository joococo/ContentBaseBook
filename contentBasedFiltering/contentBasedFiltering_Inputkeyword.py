...
cursor.execute("SELECT book.bookInform FROM book, student, checkout WHERE book.bookid = checkout.bookid AND 20210796 = checkout.studentid;")
readbooks = cursor.fetchall()

cnt = 0
for i in range(len(readbooks)):
    userBook.append(readbooks[i][0])

print(userBook)


book_descriptions = [book["description"] for book in bookData]
descriptions = book_descriptions + userBook
...
user_book_nouns = [extract_nouns(book) for book in userBook]
#print(user_book_nouns)
user_tfidf = tfidf_vectorizer.transform(user_book_nouns)
...
for idx in sorted_indices:
    if bookData[idx]["description"] not in userBook:
        recommendations.append(bookData[idx]["name"])

print("추천 도서:")
for rec in recommendations[:10]:  # 강의 몇 개 추천
    print(rec)
